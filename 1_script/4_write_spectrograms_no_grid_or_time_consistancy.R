...
out.root="C:/Users/jeremiah.kennedy/Documents/Working/PMRA_meeting_output/Spectrograms/" # output directory where results are kept
...
meta$Date = paste0(meta$year,formatC(meta$month,width = 2, flag = 0),formatC(meta$day,width = 2,flag = 0)) # get date for visually helpful processing
...
meta$size = as.numeric(file.size(meta$full))
...
meta_2 = meta %>% group_by(night.ID) %>% mutate(night.seq = order(time)) %>% arrange(night.ID,night.seq) 
...
for (site in unique(meta_2$prefix)){
  ...
  all_nights = unique(dat_in$night.ID)
    ...
    for (j in 1:length(all_nights)){
      ...
      dat_ret = dat_in[dat_in$night.ID %in% grp_night,]
